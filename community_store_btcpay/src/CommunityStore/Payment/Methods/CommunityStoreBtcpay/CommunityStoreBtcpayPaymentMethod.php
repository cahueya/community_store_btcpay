...
class CommunityStoreBtcpayPaymentMethod extends StorePaymentMethod
{
    ...
    public static function validateCompletion()
    {
        ...
        if (true === empty($payload)) {
            Log::addError("Error. Could not decode the JSON payload from BTCPay.\n");
        ...
        if (true === empty($payload->invoiceId)) {
            Log::addError("Error. Invalid BTCPayServer payment notification message received - did not receive invoice ID.\n");
        ...
        if ($payload->type == "InvoiceInvalid") {
            if ($order) {
                ...
                Log::addInfo("Order was cancelled" . $payload->invoiceId . " Type: " . $payload->type . " Price: " . $invoicePrice . " E-Mail: " . $buyerEmail . "\n");
